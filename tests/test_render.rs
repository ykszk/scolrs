...
fn test_render_cases() -> Result<()> {
    ...
    let output = Command::new(&bin_path)
        .arg("curve")
        .arg(test_dir.join("case1/frontal.json"))
        .output()?;
    let curve_set_path = tmp_dir.join("case1_frontal.json");
    std::fs::write(&curve_set_path, output.stdout)?;
    let colors = test_dir.join("colors.yaml");
    let line_colors = test_dir.join("line_colors.csv");
            ...
            .args(
                [
                    ...
                    vec![
                        ...
                        test_dir.join(format!("case1/{stem}.json")).as_os_str(),
