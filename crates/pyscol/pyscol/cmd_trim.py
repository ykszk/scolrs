...
def add_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        '-j', '--jobs', help='Number of jobs. default: %(default)s', type=int, default=-1
    )
...
def process_file(input: Path, output: Path, tags: list, predicate_sources: list[pyscol.TrimPredicateSource], percentile: float):
    import piexif
    import pydicom
    ...
    crop_param = CropParams(original_size=list(arr.shape), window_minmax=minmax.tolist(), crop_tlbr=np.stack(bbox).ravel().tolist())
...
def main(args: argparse.Namespace):
    ...
    if args.input.is_dir():
        ...
        Parallel(n_jobs=args.jobs)(
            delayed(process_file)(input, args.output / input.with_suffix('.jpg').name, tags, pred_sources, args.percentile)
